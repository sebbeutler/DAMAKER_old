...
from tiffChannel import TiffChannel, TiffFrame
...
class ChannelOperation(Operation):
    def __init__(self, input: list[type], output: type, channel: TiffChannel):
...
class ChannelLoad(ChannelOperation):
    def __init__(self, filename: str, channel: int=0):
        super().__init__([str], TiffChannel, TiffChannel())
        self.filename = filename
        self.channel_id = channel
    
    def run(self):
        self.channel.copyFrom(openTiff_hybrid(self.filename)[self.channel_id])
        return self.channel

class ChannelInvert(ChannelOperation):
    def __init__(self, chn: TiffChannel):
        super().__init__([TiffChannel], TiffChannel, chn)
...
class ChannelCrop(ChannelOperation):
    def __init__(self, chn: TiffChannel, x1: int, y1: int, x2: int, y2: int):
        super().__init__([TiffChannel, int, int, int, int], TiffChannel, chn)
        self.p1 = (x1, y1)
        self.p2 = (x2, y2)
...
class ChannelRotate(ChannelOperation):
    def __init__(self, chn: TiffChannel, angle: float, interpolation: str="bicubic"):
        super().__init__([TiffChannel, float, str], TiffChannel, chn)
        self.angle = angle
...
class PixelIntensity(Operation):
    def __init__(self, chn: TiffChannel):
        super().__init__([TiffChannel], list)
...
class ChannelPlot(Operation):
    def __init__(self, channel: OperationOutput):
        ...
        if type(channel) is TiffChannel:
            input = OperationOutput(input)
        
        if type(channel) is not OperationOutput:
            raise TypeError("the ChannelPlot operation requires a TiffChannel or a OperationOutput that correspond to a TiffChannel")
...
class ChannelZProjectionMax(Operation):
    def __init__(self, channel: TiffChannel):
        super().__init__([TiffChannel], TiffFrame)
        
        if type(channel) is TiffChannel:
            input = OperationOutput(input)
        
        if type(channel) is not OperationOutput:
            raise TypeError("the ChannelZProjectionMax operation requires a TiffChannel or a OperationOutput that correspond to a TiffChannel")
...
class ChannelZProjectionMin(Operation):
    def __init__(self, channel: TiffChannel):
        ...
        if type(channel) is not OperationOutput:
            raise TypeError("the ChannelZProjectionMin operation requires a TiffChannel or a OperationOutput that correspond to a TiffChannel")
...
class ChannelZProjectionMean(Operation):
    def __init__(self, channel: TiffChannel):
        ...
        if type(channel) is not OperationOutput:
            raise TypeError("the ChannelZProjectionMean operation requires a TiffChannel or a OperationOutput that correspond to a TiffChannel")
...
class ChannelAND(ChannelOperation):
    def __init__(self, *channels, threshold: int=1):
        super().__init__([list, int], TiffChannel, TiffChannel())
...
class ChannelOR(ChannelOperation):
    def __init__(self, *channels):
        super().__init__([list], TiffChannel, TiffChannel())
...
class ChannelAdjustContrast(ChannelOperation):
    def __init__(self, channel: TiffChannel, brightness: int, contrast: int):
        super().__init__([TiffChannel, int, int], TiffChannel, channel)
...
class ChannelThreshold(ChannelOperation):
    def __init__(self, channel: TiffChannel, t_min: int, t_max: int):
        super().__init__([TiffChannel, int, int], TiffChannel, channel)
...
class ChannelResliceTop(ChannelOperation):    
    def __init__(self, channel: TiffChannel):
        super().__init__([TiffChannel], TiffChannel, TiffChannel())
...
class ChannelReverse(ChannelOperation):
    def __init__(self, channel: TiffChannel):
        super().__init__([TiffChannel], TiffChannel, channel)
...
class ChannelVolume(Operation):
    def __init__(self, channel: TiffChannel, min_obj_size=0):
        super().__init__([TiffChannel, int], float)
...
class ChannelVolumeArray(Operation):
    def __init__(self, channel: TiffChannel, threshold: int=0):
        super().__init__([TiffChannel, int], list)
...
class ChannelExportOBJ(Operation):
    def __init__(self, channel: TiffChannel, filename="", step_size=2):
        super().__init__([TiffChannel, str, int], None)
...
if __name__ == '__main__':
    ...
    chn2 = openTiff_hybrid("../resources/Threshold3.4UserAveragedC1E2.tif")[0]
